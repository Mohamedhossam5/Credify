...
const AdminNavbar: React.FC<AdminNavbarProps> = ({ onToggleSidebar, onToggleTheme, theme }) => {
  ...
  const [readNotifs, setReadNotifs] = useState<Set<string>>(() => {
    try { return new Set(JSON.parse(localStorage.getItem('admin_read_notifs') || '[]')); } catch { return new Set(); }
  });
  const [deletedNotifs, setDeletedNotifs] = useState<Set<string>>(() => {
    try { return new Set(JSON.parse(localStorage.getItem('admin_deleted_notifs') || '[]')); } catch { return new Set(); }
  });
  ...
  const handleMarkAsRead = (id: string, e: React.MouseEvent) => {
    ...
    setReadNotifs(prev => {
      ...
      localStorage.setItem('admin_read_notifs', JSON.stringify([...next]));
      return next;
    });
  };
  ...
  const handleDeleteNotif = (id: string, e: React.MouseEvent) => {
    ...
    setDeletedNotifs(prev => {
      const next = new Set(prev).add(id);
      localStorage.setItem('admin_deleted_notifs', JSON.stringify([...next]));
      return next;
    });
  };
  ...
  const visibleNotifications = notifications.filter(n => !deletedNotifs.has(n.id));
  const unreadCount = visibleNotifications.filter(n => !readNotifs.has(n.id)).length;

  const handleMarkAllRead = () => {
    const newRead = new Set(readNotifs);
    visibleNotifications.forEach(n => newRead.add(n.id));
      ...
      <div className="topbar-right">
        ...
        <div style={{ position: 'relative' }} ref={dropdownRef}>
          <button className="topbar-btn" onClick={toggleNotif} title="Notifications" id="notif-btn">
            ...
            <span className="notif-dot" id="notif-dot" style={{ display: unreadCount > 0 ? 'block' : 'none' }}></span>
          </button>

          <div className="notif-dropdown" id="notif-dropdown" style={{ display: notifOpen ? 'block' : 'none' }}>
            <div className="notif-header">
              <span className="notif-header-title">Notifications {unreadCount > 0 && `(${unreadCount})`}</span>
              <div style={{ display: 'flex', gap: '12px' }}>
                {unreadCount > 0 && <span className="notif-clear" onClick={handleMarkAllRead} style={{ color: 'var(--teal)' }}>Mark all read</span>}
            ...
            <div id="notif-list" style={{ maxHeight: '350px', overflowY: 'auto' }}>
              {visibleNotifications.length === 0 ? (
                ...
                visibleNotifications.map(n => {
                  ...
                  return (
                    <div className="notif-item" key={n.id} onClick={() => { navigate(n.link); setNotifOpen(false); }} style={{ cursor: 'pointer', opacity: isRead ? 0.7 : 1 }}>
                      <div className="notif-dot-icon" style={{ background: n.color }}></div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div className="notif-text" style={{ fontWeight: isRead ? 400 : 600 }}>{n.title}: {n.desc}</div>
                        <div className="notif-time">{n.time}</div>
                      </div>
                      <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginLeft: '12px' }}>
                        {!isRead && (
                          <button onClick={(e) => handleMarkAsRead(n.id, e)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--teal)', padding: '4px' }} title="Mark as read">
                            <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>
                          </button>
                        )}
                        <button onClick={(e) => handleDeleteNotif(n.id, e)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--danger, #ef4444)', padding: '4px' }} title="Delete">

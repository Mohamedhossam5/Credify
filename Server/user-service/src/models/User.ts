...
class User {
  static async create(input: CreateUserInput): Promise<UserRecord> {
    const query = `
      ...
      RETURNING id, first_name, last_name, email, id_number, birthdate, address, kyc_status, created_at
    `;
    const values = [
  ...
  static async findById(id: number): Promise<UserRecord | null> {
    const { rows } = await pool.query(
      "SELECT id, first_name, last_name, email, id_number, birthdate, address, kyc_status, created_at FROM users WHERE id = $1",
      [id]
    );
    return rows[0] || null;
  }

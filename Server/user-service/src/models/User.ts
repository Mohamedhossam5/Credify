...
class User {
  ...
  static async findById(id: number): Promise<UserRecord | null> {
    const { rows } = await pool.query(
      "SELECT id, first_name, middle_name, last_name, email, phone_number, gender, id_number, birthdate, address, phone_verified, email_verified, kyc_status, role, created_at FROM users WHERE id = $1",
      [id]
    );
    return rows[0] || null;
  }
